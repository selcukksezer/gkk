...
function determineRarityOutcome(facilityLevel: number, suspicionLevel: number): string {
...
Deno.serve(async (req) => {
  try {
    ...
    if (recipeError || !recipe) {
      return new Response(
        JSON.stringify({ success: false, error: 'Recipe not found for this facility' }),
        { status: 404, headers: { 'Content-Type': 'application/json' } }
      )
    }

    // Check facility level requirement
    if (facility.level < recipe.required_level) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: `Facility level ${facility.level} is too low. Required: ${recipe.required_level}` 
        }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      )
    }
    ...
    // Validate materials
    for (const [materialId, required] of Object.entries(requiredMaterials) as [string, number][]) {
    ...
    if (queueItems && queueItems.length >= 10) {
      return new Response(
        JSON.stringify({ success: false, error: 'Queue is full (max 10 items)' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      )
    }
