...
func _populate_shop_items() -> void:
	...
	for item_data in all_items:
		...
		if card.has_signal("item_selected"):
			card.item_selected.connect(_on_item_purchase.bind(item_data))
...
func _on_item_purchase(item_data: ItemData) -> void:
	...
	# For testing: Directly add to inventory without API call
	print("[Shop] Purchasing item for testing: ", item_data.name)
